...
CliquePhasedLocalSearch::CliquePhasedLocalSearch(vector<vector<int>>       &vvAdjacencyMatrix, 
                                                 vector<vector<int>> const &vAdjacencyArray, 
                                                 vector<double>      const &vVertexWeights)
: PhasedLocalSearch(vAdjacencyArray,vVertexWeights)
, m_vvAdjacencyMatrix(std::move(vvAdjacencyMatrix))
{
    SetName("pls-clique");
...
//DS: Since always selecting from maxdegree vertices, can maintain in separate space.
...
#else
////    for (int const zero_vertex : m_NotAdjacentToZero) {
    for (std::vector<int>::iterator it = m_NotAdjacentToZero.begin(); it != m_NotAdjacentToZero.end(); it++) {
        int const zero_vertex(*it);
        if (!m_vvAdjacencyMatrix[vertex][zero_vertex]) {
            m_NotAdjacentToZero.Remove(zero_vertex);
            it--;
            zeroDiffVertices.push_back(zero_vertex);
        }
    }
#endif // SLOW
...
#else
////    for (int const one_vertex : m_NotAdjacentToOne) {
    for (std::vector<int>::iterator it = m_NotAdjacentToOne.begin(); it != m_NotAdjacentToOne.end(); it++) {
        int const one_vertex(*it);
        if (!m_vvAdjacencyMatrix[vertex][one_vertex]) {
            it--;
            m_NotAdjacentToOne.Remove(one_vertex);
        }
    }
#endif // SLOW
...
void CliquePhasedLocalSearch::InitializeFromK()
{
    ...
    for (int vertex = 0; vertex < m_vAdjacencyArray.size(); ++vertex) {
...
#ifdef SLOW
        // TODO/DS: Slow if many neighbors.
...
#else
        size_t missing_neighbor_count(0);
        for (int const vertex_in_k : m_K) {
            if (!m_vvAdjacencyMatrix[vertex][vertex_in_k]) {
                missing_neighbor_count++;
                if (missing_neighbor_count == 2) {
                    break;
                }
            }
        }
        ...
        if (missing_neighbor_count == 0) {
            m_NotAdjacentToZero.Insert(vertex);
            m_bCheckZero = m_bCheckZero || !m_U.Contains(vertex);
        } else if (missing_neighbor_count == 1) { 
            m_NotAdjacentToOne.Insert(vertex);
            m_bCheckOne = m_bCheckOne || !m_U.Contains(vertex);
        }
    }
