...
#        sys.exit(1)
    ...
    #average time
    average_time = (sum([float(x.strip("s")) for x in hash_list["time(s)"]]) + sum([float(x.strip("s")) for x in hash_list["reduced-time(s)"]]))/len(hash_list["time(s)"])
    reduced_array.append("{0:.4f}".format(average_time))
